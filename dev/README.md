...
`provider-seaweedfs`. It installs the latest released OpenEverest v2 core and
...
- Apply an example Instance to exercise the provider:
  ...
  ```bash
  kubectl apply -f examples/seaweedfs-standalone.yaml
...
| `PROVIDER_NAMESPACE` | `default` | Namespace for the provider + SeaweedFS operator. |
